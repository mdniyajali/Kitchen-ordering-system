...
def MenuItemUpdateView(request,pk):
    if request.user.is_authenticated == True:
        ...
        form = MenuItemForm(instance=item_obj)
        if request.method == "POST":
            form = MenuItemForm(request.POST,request.FILES,instance=item_obj)
            if form.is_valid():
                form.save()
                return redirect('moms:profile')
        context = {
...
def feedback_view(request):
    if request.method == 'POST':
        ...
        return redirect('mom:feedback')  # Correct usage of namespace here
    else:
